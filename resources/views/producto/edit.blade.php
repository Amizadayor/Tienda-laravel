        ...
        <div class="bg-white rounded-lg shadow-lg">
            <div class="p-4">
                <div class="flex justify-between items-center">
                    <h2 class="text-2xl font-bold">{{ __('Editar Producto') }}</h2>
                ...
                <form action="{{ route('producto.update', $producto->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <select name="marca_id" id="marca_id"
                                ...
                                @foreach ($marcas as $marca)
                                    <option value="{{ $marca->id }}"
                                        {{ $producto->marca_id == $marca->id ? 'selected' : '' }}>{{ $marca->nombre }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        <div>
                            <label for="cantidad_disponible"
                                class="block font-bold">{{ __('Cantidad Disponible') }}</label>
                            <input type="number" name="cantidad_disponible" id="cantidad_disponible"
                                class="w-full border-gray-300 rounded-lg focus:border-blue-500 focus:ring-blue-500"
                                value="{{ $producto->cantidad_disponible }}" required>
                        </div>
                        <div>
                    ...
                    <div class="mt-4">
                        ...
                        <textarea name="descripcion" id="descripcion" rows="4"
                            class="w-full border-gray-300 rounded-lg focus:border-blue-500 focus:ring-blue-500">{{ $producto->descripcion }}</textarea>
                    </div>

                    <div class="mt-6">
                        <button type="submit"
                            class="bg-blue-500 hover:bg-blue-700 text-white rounded-lg px-4 py-2 font-bold">
